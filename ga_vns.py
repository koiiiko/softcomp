...
class ClusterBasedDroneRoutingHybrid:
    drone_speed = 30
    max_flight_time = 170
    ...
    def __init__(self, csv_file=None, road_points=None, n_drones=None,
                 vns_on_children=True, vns_budget=50):
        ...
        self.vns_on_children = vns_on_children
        self.vns_budget = vns_budget
    ...
    def _local_search(self, route, max_no_improve=50):
        best = route
    ...
    def solve_tsp_hybrid(self, cluster_id, hotspot_indices=None, population_size=40, generations=100, mutation_rate=0.1):
        ...
        - VNS refines each child if vns_on_children=True
        """
        ...
        for _ in range(generations):
            ...
            for _ in range(population_size):
                ...
                # ⚡ Hybridization happens here:
                if self.vns_on_children:
                    child = self._local_search([start_point] + child + [start_point], max_no_improve=self.vns_budget)[1:-1]
    ...
    def visualize_cluster_routes(self, all_routes):
        ...
        for cid, routes in all_routes.items():
            ...
            for d_idx, route in enumerate(routes):
                ...
                linestyle = linestyles[d_idx % len(linestyles)] if num_drones > 1 else '-'
                marker = markers[d_idx % len(markers)] if num_drones > 1 else 'o'

                plt.plot(lons, lats, linestyle=linestyle, marker=marker,
                         color=color, label=f'Cluster {cid} Drone {d_idx+1}',
                         linewidth=2, markersize=6)

        plt.title('Drone Routes (Hybrid GA + VNS)')
