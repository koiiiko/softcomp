...
from new_ga import ClusterBasedDroneRouting
...
class ClusterBasedDroneRoutingHybridTabuOptimized:
    ...
    def visualize_cluster_routes(self, all_routes,
                             ...
                             output_path="new_forest_fire_clusters_with_drone_routes.html"):
...
class ClusterBasedDroneRoutingHybridTabuMelawi:
    ...
    def visualize_cluster_routes(self, all_routes):
        ...
        # 🎨 Warna cluster (bisa kamu ubah sesuka hati)
        cluster_color_palettes = {
            0: ["#A93226", "#E74C3C"],  # merah tua & merah muda
            1: ["#1E8449", "#52BE80"],  # hijau tua & hijau muda
            2: ["#2471A3", "#85C1E9"],  # biru tua & biru muda
            3: ["#AF7AC5", "#fa9725"],  # ungu tua & oranye lembut
            4: ["#5D6D7E", "#CCD1D1"],  # abu tua & abu muda
        }

        # Gaya garis untuk drone dalam cluster
        line_styles = ['solid', 'dashed', 'dashdot', 'dotted']  # bisa ditambah jika drone > 4
        ...
        for cid, routes in all_routes.items():
            ...
            for d_idx, route in enumerate(routes):
                ...
                # ambil warna dan gaya garis untuk drone ini
                color = palette[d_idx % len(palette)]
                style = line_styles[d_idx % len(line_styles)]
                ...
                plt.plot(lons, lats, 'o-', color=color, linestyle=style,
                        label=f'Cluster {cid+1} Drone {d_idx+1}',
                        linewidth=2.3, markersize=6, alpha=0.9)

        plt.title('Hybrid GA + Tabu Search (Melawi)', fontsize=14, fontweight='bold')
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        plt.legend()
        plt.grid(True, alpha=0.3)
